...
public class RewardsDAO {
    ...
    public String insertarCodigoXCliente(int idCliente, int descuento, String codpromo) {
        ...
        try (Connection conn = Conexion.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            e.printStackTrace();
            // Aquí podrías agregar un registro de log si fuera necesario
        }
    ...
    public int invalidarCodPromo(int idCliente) {
        ...
        try (Connection conn = Conexion.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            // Ejecutar la operación de eliminación y obtener el número de filas afectadas
            r = ps.executeUpdate();
    ...
    public int canjearCodPromo(int idCliente, String codpromo) {
        ...
        try (Connection conn = Conexion.getConnection(); PreparedStatement ps = conn.prepareStatement(verificarSql); PreparedStatement psUpdate = conn.prepareStatement(canjearSql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    if (fechaExpiracion != null && fechaExpiracion.isAfter(LocalDate.now()) && estado == 1) {
                        ...
                        psUpdate.setInt(1, 0); // Cambiar el estado a 0 (usado)
                        psUpdate.setDate(2, Date.valueOf(LocalDate.now())); // Registrar la fecha de uso
                        psUpdate.setInt(3, idCliente);
                        psUpdate.setString(4, codpromo);

                        psUpdate.executeUpdate(); // Ejecuta la actualización
                    } else {
